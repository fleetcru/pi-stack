...
export class PiServerClient {
  ...
  getSessionGitStatus(id: string): Promise<GitStatusResponse> {
    return this.request<GitStatusResponse>(`/v1/sessions/${encodeURIComponent(id)}/git/status?format=json`).catch((error: unknown) => {
      ...
      if (error instanceof PiServerApiError && error.status === 422) {
