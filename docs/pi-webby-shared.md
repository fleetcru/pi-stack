...
- `client.ts` — `PiServerClient`, a thin typed fetch wrapper over pi-server's REST API: capabilities, sessions CRUD, RPC/prompt, git status/commit/push, file tree/content, workers, daemon status, and WS-ticket requests. Model and allowed-directory requests accept a worker ID and use the worker-scoped proxy endpoints for remote machines. Exports re-typed aliases (`ApiSession`, `RpcCommand`, …) and `PiServerApiError` with status/code detail. Base URL defaults to `VITE_PI_SERVER_URL` then `localhost:3142`; token stays out of storage unless the caller opts in.
