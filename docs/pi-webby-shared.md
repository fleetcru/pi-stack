...
TypeScript library (`@pi-stack/webby-shared`) containing everything the browser and desktop clients share: the API client, the WebSocket wrapper, React Query hooks, generated types, the Zustand store factory, and session-workspace timeline logic. `dist/` is build output; `src/` is the source of truth. CI installs this package's dependencies before checking either app because TypeScript resolves the shared sibling sources from this directory, not from each app's `node_modules`.
...
- `client.ts` — `PiServerClient`, a thin typed fetch wrapper over pi-server's REST API: capabilities, sessions CRUD, RPC/prompt, git status/commit/push, file tree/content, workers, trusted devices, bearer-authenticated administration state/settings, server performance sampling (`performance()` → `PerformanceReport`/`PerformanceSeries`/`PerformanceSample`/`RequestPerformance`), daemon status, and WS-ticket requests. Model and allowed-directory requests accept a worker ID and use the worker-scoped proxy endpoints for remote machines. Exports re-typed aliases (`ApiSession`, `AdminSettings`, `AdminState`, `TrustedDevice`, …) and `PiServerApiError` with status/code detail. Base URL defaults to `VITE_PI_SERVER_URL` then `localhost:3142`; token stays out of storage unless the caller opts in.
...
- `hooks.ts` / `hooks-base.ts` / `session-history.ts` — React Query hooks built on the client: `usePiServerClient` (from provider context), worker-aware `useAvailableModels` and `useDirectoryRoots` queries, `useSessionData`, `useSessionHistory` (session-scoped infinite query with longer `gcTime`; re-fetches on socket gaps), `usePrefetchSessionHistory` / `useSessionHistoryPrefetchHandlers` (debounced sidebar hover/focus prefetch of the newest history page), file/git hooks, `useSchedulerStatus`, and `useActiveSessionSocket` which owns the selected session's long-lived socket.
