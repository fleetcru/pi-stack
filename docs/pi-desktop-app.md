...
### Components
- `components/workspace-shell.tsx` — the workspace reveals after the first React frame; worker, global-session, and machine-session inventory queries start on the next frame so health and primary sessions can render first. Its server toolbar links to the administration route. Like Webby, it owns exactly one server-wide status socket for the selected server (`useServerStatusSocket`) and shows the compact Active runs section (scheduler runs merged with live runtime status, Open and Cancel actions, cancel disabled with a hint for non-local active runs).
