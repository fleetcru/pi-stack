...
- `session-inspector.tsx` — right panel: session details, file tree browser with file preview, git status/branches/worktrees, and the guided commit/push flow (`resolveGitQuickAction` computing Commit → Commit & push → Push → blocked, with the GitHub compare link when `githubRepo` is known). Its Metrics tab mounts `session-performance-panel.tsx` only while selected and shows the managed local Pi process's current CPU/RAM, retained peaks, lightweight history charts, status, and PID from `GET /v1/performance`.
- `sidebar-tree.tsx` — grouped session list (local, remote, relay, machine-discovered) with search and per-session context menus. Worker, project, and session rows use roomier targets, project session counts, clearer selection, and accessible hover/focus pin controls. Session rows debounce-prefetch history on hover/focus so switching often hits a warm React Query cache.
- `quick-session-composer.tsx` — the primary new-session surface (empty workspace and dialog). Wide prompt card with searchable model picker, thinking/effort, worker, and project root. Collapsible "More options" folds in title, isolated worktree, batch count 1–12, worker-aware folder browse, and advanced args/labels. Prompt is optional when More options is open or when embedded in the create dialog; sidebar "+" focuses/expands this same composer (dialog wrapper when a session is already selected).
- `new-session-dialog.tsx` — thin Dialog shell around `QuickSessionComposer` (`variant="dialog"`, More options open by default) so "+" and empty-workspace create stay one component.
...
- `server-admin-page.tsx` — full server administration route (`/admin`), mirrored with Desktop. It switches among configured servers and manages overview/warnings, all persisted settings (live vs restart-required), remote workers, and trusted-device credentials through the bearer-authenticated `/v1/admin/*`, `/v1/workers/*`, and `/v1/devices/*` APIs. A Performance tab (`performance-panel.tsx`) polls `GET /v1/performance` every 15 s while the tab is visible, rendering server CPU/RSS/heap/request-rate cards, CPU and memory history sparklines (inline SVG, no chart library), scheduler/request health, a per-session CPU/RAM table with running status, and a route request table, with loading, error, and empty states.
